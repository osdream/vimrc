...
function! <SID>GetCoffeeFileHeader()
  ...
  let doc = doc. " \n \n \n"
  let doc = doc. " \n \n"
  let doc = doc. "__author__ = '".<SID>GetUserName()." <".<SID>GetEmail().">'\n"
  let doc = doc. "__date__ = '".<SID>GetDate()."'\n"
  let doc = doc. "__revision = '\$Revision\$'\n"
  return doc
endfunction
...
function! <SID>GetPythonFileHeader()
  let doc = ""
  let doc = doc. "#!/usr/bin/env python\n"
  let doc = doc. "#!-*- coding:utf-8 -*-\n"
  let doc = doc. "#!vim: set ts=4 sw=4 sts=4 tw=100 :\n"
  let doc = doc. "# ***************************************************************************\n"
  let doc = doc. "# \n"
  let doc = doc. "# Copyright (c) ".<SID>GetYear()." Baidu.com, Inc. All Rights Reserved\n"
  ...
  let doc = doc. "# **************************************************************************/\n"
  let doc = doc. " \n \n \n"
  let doc = doc. "import os\n"
  let doc = doc. "import sys\n"
  let doc = doc. " \n \n"
  let doc = doc. "__author__ = '".<SID>GetUserName()." <".<SID>GetEmail().">'\n"
  let doc = doc. "__date__ = '".<SID>GetDate()."'\n"
  let doc = doc. "__revision = '\$Revision\$'\n"
...
function! <SID>GetDoxFH(type)
  ...
  elseif (a:type == 123)
    let doc = <SID>GetCoffeeFileHeader()
  elseif (a:type == 0)
...
function! <SID>InsertCoffeeHead()
  call <SID>GetDoxFH(123)
  let idx = 5
  while (idx > 0)
    call append(line('$') - 1, "")
    let idx = idx - 1
